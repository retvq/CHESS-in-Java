...
public class UserInterface extends JPanel implements MouseListener, MouseMotionListener {       //This class contains our GUI and everything the user needs to interface with the chess board
    ...
    public void paintComponent(Graphics g){
        ...
        this.setBackground(Color.DARK_GRAY);                // sets the background to black. dont think its necessary because i made it non-resizable but jic
        this.addMouseListener(this);                //so user can use the mouse
        this.addMouseMotionListener(this);

        for ( int i  =0; i < 64; i += 2){
            setVisible(true);                                                               //makes visible
            setSize(1800,1800) ;                                                 // sets the size of the board
            g.setColor(new Color(232, 235, 239));                                            // sets the color for the lighter tiles ( a lightish green color)
            g.fillRect((i%8 + (i/8)%2)*tile, (i/8)*tile, tile, tile);                           //creates the light tiles (50% of the board)
            g.setColor(new Color(125, 135, 150));                                     //sets the color for the dark tiles ( reddish color)
